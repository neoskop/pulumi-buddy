...
export interface ActionEOSState {
    project_name: string;
    pipeline_id: number;
    /**
    ...
    /**
     * The containers with the services that will be attached to this environment. Available types: `NODEOS`, `MYSQL`, `MONGO_DB`, `MARIADB`, `POSTGRE_SQL`, `REDIS`, `MEMCACHED`, `ELASTICSEARCH`.
     */
    services?: Service[];
...
export type ActionEOSArgs = AsInputs<ActionEOSState>;

export interface ActionEOSProps {
    url: string;
    html_url: string;
    action_id: number;
    docker_image_name: string;
 ...
 */
export class EOS extends CustomResource {
    static __pulumiType = 'buddy:action:EOS';

    static get(name: string, id: Input<ID>, state?: Partial<ActionEOSState>, opts?: CustomResourceOptions) {
        return new EOS(name, state as any, { ...opts, id });
    }

    static isInstance(obj: any): obj is EOS {
        if (null == obj) {
            return false;
        }

        return obj['__pulumiType'] === EOS.__pulumiType;
    }
    ...
    constructor(name: string, argsOrState: ActionEOSArgs | ActionEOSState, opts?: CustomResourceOptions) {
        const inputs: Inputs = {};
        if (!opts) {
            opts = {};
        }
        ...
        if (opts.id) {
            const state = argsOrState as ActionEOSState | undefined;
            ...
            const args = argsOrState as ActionEOSArgs | undefined;
        ...
        super(EOS.__pulumiType, name, inputs, opts);
    }
}
