...
export interface CopyFilesState {
    ...
    /**
     * The object with the id of the pipeline from which files will be copied. If not set, the source pipeline will be the current pipeline.
     */
    ...
    /**
     * The object with the id of the pipeline to which files will be copied. If not set, the target pipeline will be the current pipeline.
     */
    target_pipeline?: PipelineRef;
...
export type CopyFilesArgs = AsInputs<CopyFilesState>;

export interface CopyFilesProps {
    url: string;
    html_url: string;
    action_id: number;
    name: string;
 ...
 */
export class CopyFiles extends CustomResource {
    static __pulumiType = 'buddy:action:CopyFiles';

    static get(name: string, id: Input<ID>, state?: Partial<CopyFilesState>, opts?: CustomResourceOptions) {
        return new CopyFiles(name, state as any, { ...opts, id });
    }

    static isInstance(obj: any): obj is CopyFiles {
        if (null == obj) {
            return false;
        }

        return obj['__pulumiType'] === CopyFiles.__pulumiType;
    }
    ...
    source_pipeline!: Output<PipelineRef | undefined>;
    target_path!: Output<string | undefined>;
    target_pipeline!: Output<PipelineRef | undefined>;
    timeout!: Output<number | undefined>;
    trigger_conditions!: Output<TriggerCondition[] | undefined>;
    variables!: Output<Variable[] | undefined>;

    constructor(name: string, argsOrState: CopyFilesArgs | CopyFilesState, opts?: CustomResourceOptions) {
        const inputs: Inputs = {};
        if (!opts) {
            opts = {};
        }
        ...
        if (opts.id) {
            const state = argsOrState as CopyFilesState | undefined;
            ...
            inputs['target_pipeline'] = state?.target_pipeline;
            inputs['timeout'] = state?.timeout;
            inputs['trigger_conditions'] = state?.trigger_conditions;
            inputs['variables'] = state?.variables;
        } else {
            const args = argsOrState as CopyFilesArgs | undefined;
            ...
            inputs['target_pipeline'] = args.target_pipeline;
        ...
        super(CopyFiles.__pulumiType, name, inputs, opts);
    }
}
