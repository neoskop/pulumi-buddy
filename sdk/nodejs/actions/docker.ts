...
export interface DockerState {
    ...
    /**
     * The name of the Amazon region. If the Amazon ECR integration has been selected, provide the region to authorize in. The full list of regions is available here.
     */
    region?: string;
...
export type DockerArgs = AsInputs<DockerState>;
 ...
 */
export class Docker extends CustomResource {
    static __pulumiType = 'buddy:action:Docker';

    static get(name: string, id: Input<ID>, state?: Partial<DockerState>, opts?: CustomResourceOptions) {
        return new Docker(name, state as any, { ...opts, id });
    }

    static isInstance(obj: any): obj is Docker {
        if (null == obj) {
            return false;
        }
    ...
    constructor(name: string, argsOrState: DockerArgs | DockerState, opts?: CustomResourceOptions) {
        const inputs: Inputs = {};
        if (!opts) {
            opts = {};
        }
        ...
        super(Docker.__pulumiType, name, inputs, opts);
    }
}
