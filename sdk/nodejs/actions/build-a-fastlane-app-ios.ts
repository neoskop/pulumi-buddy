...
export interface BuildAFastlaneAppiOSState {
    project_name: string;
    pipeline_id: number;
    /**
...
export type BuildAFastlaneAppiOSArgs = AsInputs<BuildAFastlaneAppiOSState>;

export interface BuildAFastlaneAppiOSProps {
    url: string;
    html_url: string;
    action_id: number;
    commands: string[];
 ...
 */
export class BuildAFastlaneAppiOS extends CustomResource {
    static __pulumiType = 'buddy:action:BuildAFastlaneAppiOS';

    static get(name: string, id: Input<ID>, state?: Partial<BuildAFastlaneAppiOSState>, opts?: CustomResourceOptions) {
        return new BuildAFastlaneAppiOS(name, state as any, { ...opts, id });
    }

    static isInstance(obj: any): obj is BuildAFastlaneAppiOS {
        if (null == obj) {
            return false;
        }

        return obj['__pulumiType'] === BuildAFastlaneAppiOS.__pulumiType;
    }
    ...
    type!: Output<'NATIVE_BUILD_MAC_FASTLANE'>;
    ...
    constructor(name: string, argsOrState: BuildAFastlaneAppiOSArgs | BuildAFastlaneAppiOSState, opts?: CustomResourceOptions) {
        ...
        if (opts.id) {
            const state = argsOrState as BuildAFastlaneAppiOSState | undefined;
            ...
            const args = argsOrState as BuildAFastlaneAppiOSArgs | undefined;
        ...
        inputs['type'] = 'NATIVE_BUILD_MAC_FASTLANE';
        inputs['url'] = undefined;
        inputs['html_url'] = undefined;
        inputs['action_id'] = undefined;

        super(BuildAFastlaneAppiOS.__pulumiType, name, inputs, opts);
    }
}
