...
export class PipelineProvider implements SubProvider {
    ...
    create(req: ServerUnaryCall<CreateRequest>, callback: sendUnaryData<CreateResponse>) {
        ...
        this.buddyApi.workspace(this.config.workspace).project(props.project_name).pipeline().create(props).then(result => {
            const response = new CreateResponse();
            response.setId(Id.stringify([['Project' as Kind, props.project_name], ['Pipeline' as Kind, result.id.toString()]]));
    ...
    delete(req: ServerUnaryCall<DeleteRequest>, callback: sendUnaryData<Empty>) {
        ...
        this.buddyApi.workspace(this.config.workspace).project(id[0][1]).pipeline(+id[1][1]).delete().then(() => callback(null, new Empty()), err => {
            if (Axios.isCancel(err)) {
                callback(new ServiceError('Canceled', status.CANCELLED), null);
    ...
    update(req: ServerUnaryCall<UpdateRequest>, callback: sendUnaryData<UpdateResponse>) {
        ...
        this.buddyApi.workspace(this.config.workspace).project(id[0][1]).pipeline(+id[1][1]).update(news).then(result => {
    ...
    read(req: ServerUnaryCall<ReadRequest>, callback: sendUnaryData<ReadResponse>) {
        ...
        this.buddyApi.workspace(this.config.workspace).project(id[0][1]).pipeline(+id[1][1]).read().then(result => {
